  ...
  return (
    <div className="page-body">
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 }}>
        <div>
          <h1 style={{ fontSize:22, fontWeight:800, color:'#e8edf8' }}>⚙️ School Setup</h1>
          <p style={{ color:'#7a90b8', fontSize:13, marginTop:4 }}>Configure your school profile and financial years</p>
        </div>
      </div>
        ...
        <div className="glass" style={{ padding:32 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
            ...
            <div className="form-group">
              ...
              <select className="form-select" value={school.school_type} onChange={e => setSchool(p=>({...p,school_type:e.target.value}))}>
      ...
      {tab === 'years' && (
        <>
          ...
          <div className="glass" style={{ padding:28, marginBottom:24 }}>
            <h2 style={{ fontSize:16, fontWeight:700, color:'#e8edf8', marginBottom:20 }}>➕ Add Financial Year</h2>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr auto', gap:16, alignItems:'end' }}>
              <div className="form-group">
                <label className="form-label">Year Label <span style={{color:'#ef4444'}}>*</span></label>
                <input className="form-input" placeholder="e.g. 2024/2025" value={newYear.year_label} onChange={e => setNewYear(p=>({...p,year_label:e.target.value}))} />
              </div>
              <div className="form-group">
                <label className="form-label">Start Date</label>
                <input className="form-input" type="date" value={newYear.start_date} onChange={e => setNewYear(p=>({...p,start_date:e.target.value}))} />
              </div>
              <div className="form-group">
                <label className="form-label">End Date</label>
                <input className="form-input" type="date" value={newYear.end_date} onChange={e => setNewYear(p=>({...p,end_date:e.target.value}))} />
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                <label style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer', fontSize:13, color:'#7a90b8' }}>
                  <input type="checkbox" checked={newYear.is_current} onChange={e => setNewYear(p=>({...p,is_current:e.target.checked}))} style={{accentColor:'#4f7ef8'}} />
                  Set as Current
                </label>
          ...
          <div style={{ background:'#0d1526', border:'1px solid #1e2d4a', borderRadius:16, overflow:'hidden' }}>
            <div style={{ padding:'14px 20px', background:'linear-gradient(135deg,#111d35,#0d1526)', borderBottom:'1px solid #1e2d4a' }}>
              <span style={{ fontSize:13, fontWeight:700, color:'#e8edf8' }}>Financial Years ({years.length})</span>
            </div>
            {years.length === 0 ? (
              <div style={{ padding:48, textAlign:'center', color:'#4a5f82' }}>
              ...
              <table className="data-grid">
                ...
                <tbody>
                  {years.map(yr => (
                    <tr key={yr.id}>
                      <td style={{ fontWeight:700, color:'#e8edf8' }}>{yr.year_label}</td>
                      <td style={{ color:'#7a90b8' }}>{yr.start_date}</td>
                      <td style={{ color:'#7a90b8' }}>{yr.end_date}</td>
                      <td>
                      ...
                      <td>
                        <div style={{ display:'flex', gap:8, justifyContent:'center' }}>
                          {!yr.is_current && (
                            <button className="btn-ghost" style={{ padding:'5px 12px', fontSize:12 }} onClick={() => setCurrent(yr)}>Set Current</button>
                          )}
                          <button className="btn-ghost" style={{ padding:'5px 12px', fontSize:12, color: yr.is_locked?'#10b981':'#f59e0b' }} onClick={() => toggleLock(yr)}>
